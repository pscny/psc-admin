class Member
  ...
  validates :email,           :presence  => true, :if => :mailing_list?
  ...
  validates :email,           :format => { :with => /.+@.+\..+/,           :message => 'please enter an email address like handle@domain.net',               :allow_blank => true }
  ...
  before_validation :format_primary_phone,   :if => :primary_phone?
  before_validation :format_secondary_phone, :if => :secondary_phone?
